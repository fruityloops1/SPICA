...
namespace SPICA.Rendering
{
    public class VertexShader : IDisposable
    {
        ...
        public ShaderNameBlock VtxNames;
        public ShaderNameBlock GeoNames;
        ...
        public VertexShader(H3DShader Shdr)
        {
            ...
            Load(SHBin, Shdr.VtxShaderIndex, Shdr.GeoShaderIndex);
        }

        public VertexShader(string name, ShaderBinary shader, int vertexProgramIdx = -1, int geomProgramIdx = -1)
        {
            Name = name;

            Load(shader, vertexProgramIdx, geomProgramIdx);
        }

        private void Load(ShaderBinary SHBin, int vertexProgramIdx = -1, int geomProgramIdx = -1)
        {
            bool HasGeometryShader = geomProgramIdx != -1;

            if (vertexProgramIdx != -1)
            {
                ...
                string Code = VtxShaderGen.GetVtxShader(vertexProgramIdx, HasGeometryShader, out VtxNames);
            ...
            if (HasGeometryShader)
            {
                ...
                string Code = GeoShaderGen.GetGeoShader(geomProgramIdx, VtxNames.Outputs, out GeoNames);
